...
const saveFeature = async () => {
  if (!form.value.app_id || !form.value.title || !form.value.release_date) {
  ...
  try {
    ...
    if (editingFeature.value) {
      const response = await featureService.update(editingFeature.value.id, form.value);
      featureId = response.data.id;
    } else {
      const response = await featureService.create(form.value);
      featureId = response.data.id;
    }
